...
public class RedisUtils {
    ...
    public static Boolean set(String key, Object value) {
        try {
            stringRedisTemplate.opsForValue().set(key, JSON.toJSONString(value),24, TimeUnit.HOURS);
            return true;
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return false;
    ...
    public static <T> Boolean updateValue(String captchaKey, T value) {
        try {
            Long expire = stringRedisTemplate.getExpire(captchaKey, TimeUnit.SECONDS);
            stringRedisTemplate.opsForValue().set(captchaKey, JSON.toJSONString(value), expire, TimeUnit.SECONDS);
            return true;
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return false;
